...
public class ActionService: ActionServiceProtocol {
    ...
    public func action(request: ActionRequest) async throws -> ActionResponse? {
        do {
            guard let url = URL(string: "\(request.route)\(request.alertId)/") else {
            ...
            if (res as? HTTPURLResponse)?.statusCode == 204 {
                print("LaunchAlert Action Response --> 204")
                return nil
            }
            if let response = try? JSONDecoder().decode(ActionResponse.self, from: data) {
                print("LaunchAlert Action Response --> 200")
                print(response)
                return response
            } else {
                print("LaunchAlert Action Response --> Decode Error")
                return nil
            }
        } catch {
            print("Failed to LaunchAlert Action Request \(error)")
            return nil
        }
    }
}
